...
export default function Exercise({ params }) {
    ...
    useEffect(() => {
        ...
        const fetchExercise = async () => {
            const response = await ky.get(
                `https://localhost:7261/api/v1/Ejercicio/buscar-ejercicio/${params.id}`
            );
    ...
    const handleEdit = (index) => {
        ...
        editedEjercicioCopy.ejercicios[index] = {
            ...editedEjercicio.ejercicios[index],
        };
    ...
    const handleRatingChange = async (newRating) => {
        try {
            //  hacer una solicitud POST al backend con la nueva puntuación
            const response = await ky.post(
                `https://localhost:7261/api/v1/Ejercicio/puntuar-ejercicio/${params.id}`,
                {
                    json: { rating: newRating },
                }
            );
            const updatedEjercicio = await response.json();
            setEjercicio(updatedEjercicio);
        } catch (error) {
    ...
    const handleSave = async () => {
        try {
            ...
            const response = await ky.post(
                `https://localhost:7261/api/v1/Ejercicio/editar-ejercicio/${userId}`,
                {
    ...
    const handleDownloadPDF = async () => {
        try {
            const response = await ky.get(
                `https://localhost:7261/api/v1/Ejercicio/pdf/${userId}/${ejercicio.id}`,
                {
                    responseType: "blob",
                }
            );
    ...
    const handleDownloadOriginalPDF = async () => {
        try {
            const response = await ky.get(
                `https://localhost:7261/api/v1/Ejercicio/pdfOriginal/${userId}/${ejercicio.id}`,
                {
                    responseType: "blob",
                }
            );
        ...
        <>
            ...
            >
                ...
                {ejercicio.ejercicios &&
                    ejercicio.ejercicios.map((ejercicioItem, index) => (
                        <Box
                            key={index}
                            sx={{
                                padding: "2%",
                        ...
                        >
                            {isEditing ? (
                                <>
                                    <TextField
                                        ...
                                        multiline
                                        onChange={(e) => {
                                            const editedEjercicioCopy = {
                                                ...editedEjercicio,
                                            };
                                            editedEjercicioCopy.ejercicios[
                                                index
                                            ].consigna = e.target.value;
                                            setEditedEjercicio(
                                                editedEjercicioCopy
                                            );
                                        }}
                            ...
                            <Rating
                                initialValue={ejercicio.puntuacion}
                ...
                <Stack direction="row" spacing={5} sx={{ width: "100%" }}>
                    ...
                    >
                        ...
                        <Typography
                            ...
                            padding="2%"
                        >
                            DESCARGAR PDF EJERCITACION{" "}
                            <span style={{ color: "green" }}>ADAPTADA</span>
                        </Typography>
                    </Button>
                    <Button
                        variant="outlined"
